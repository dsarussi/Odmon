...
Rules detect a court proceeding number in the subject, resolve it from Hozlap/UserData to an Odcanit case, parse the client number, and route to Employee A (`5,8,23,253,101,3`), Employee B (`2,15`), or Employee C (other clients). Dry-run logs `DryRunWouldForward`. Test forwarding is restricted to the approved test mailbox (shown here as `monitor@odmon.example`). Real forwarding requires `RealForwardEnabled=true`, `DryRun=false`, and applies loop guards for mailbox owner, existing To/Cc recipient, reply/forward subject, sender, and automation-generated mail.
