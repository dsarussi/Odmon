...
ODMON הוא שירות אוטומטי שתפקידו לסנכרן נתוני תיקים ממערכת Odcanit (מערכת ניהול תיקים משפטיים) אל לוח Monday.com, וכן לקלוט מסמכים מלוחות שאלונים ב-Monday חזרה לתוך Odcanit. המערכת פותרת את הבעיות הבאות:
...
- עדכונים ידניים — ללא ODMON, עדכון שדות כגון תאריך דיון, שופט, שם בית משפט, סכומי נזק וסטטוסים נעשה ידנית ונוטה לטעויות. ODMON מבצע זאת באופן אוטומטי ודטרמיניסטי.
...
ODMON אינו כותב ל-Odcanit בדרך כלל. חריגים:
- כתיבת תשובת לקוח (אישור/דחייה לדיון) באמצעות Stored Procedure ייעודי.
- קליטת מסמכים מ-Monday — יצירת רשומת מסמך ב-Odcanit באמצעות dbo.ProcDocuments_AddNewDocument עם MoveFile=3 (ללא xp_cmdshell), והעתקת הקובץ הפיזי לנתיב שהוחזר מהפרוצדורה.
...
- MondayDocumentImports — מעקב קליטת מסמכים מ-Monday ל-Odcanit (כולל מניעת כפילויות)
...
### שלב 8: קליטת מסמכים מ-Monday ל-Odcanit

המערכת כוללת צינור קליטת מסמכים מלוח שאלון ב-Monday (לוח "שאלון פתיחת לקוח") לתוך Odcanit:

1. **סריקה** — סורקת פריטים בלוח השאלון ובודקת עמודות קבצים (תצהיר ויפוי כח, אישור ניהול חשבון).
2. **זיהוי תיק** — מזהה את התיק המקושר דרך עמודת קשר ללוח התיקים הראשי, קוראת את מספר התיק (TikVisualID), ופותרת ל-TikCounter מטבלת dbo.MainTik (תומכת בעמודות TikCounter ו-Counter).
3. **הורדה** — מורידה את הקובץ מ-Monday לתיקיית Inbox מקומית בזרימה (streaming), ללא טעינה מלאה לזיכרון.
4. **יצירת רשומה ב-Odcanit** — קוראת ל-dbo.ProcDocuments_AddNewDocument עם MoveFile=3. הפרוצדורה יוצרת שורת מסמך ומחזירה DocCounter ו-DestPath — ללא הפעלת xp_cmdshell.
5. **העתקה** — מעתיקה את הקובץ מה-Inbox ל-DestPath שהוחזר מהפרוצדורה.
6. **אימות** — בודקת שהקובץ ביעד קיים וגודלו תואם למקור.
7. **ניקוי** — מוחקת את קובץ ה-Inbox לאחר אימות מוצלח.

**מניעת כפילויות:** טבלת MondayDocumentImports ב-IntegrationDb שומרת אינדקס ייחודי על (MondayQuestionnaireItemId, ColumnId, AssetId). לא ייווצר מסמך כפול ב-Odcanit עבור אותו קובץ.

**ניסיון חוזר:** כל שלב נשמר בטבלה. בעת ניסיון חוזר, המערכת ממשיכה מהשלב האחרון שהצליח (למשל: אם הפרוצדורה כבר רצה בהצלחה, תדלג ישר להעתקה).

**התראות:** כשלון בקליטת מסמך מפעיל שליחת דוא"ל דחוף עם פרטים מלאים (TikVisualID, TikCounter, AssetId, שם קובץ, שגיאה).
...
- קיימים נתוני בית משפט מאירועי יומן (EffectiveCourtCity — לצורך gating בלבד, לא למיפוי עמודה)
...
- **MondayDocumentImports** — טבלת מעקב קליטת מסמכים. כוללת סטטוס, DocCounter, DestPath, ודגל מניעת כפילויות.
- **HearingChecksum** — חתימה של שדות הדיון. שינוי בחתימה מעדכן את Monday.
- **Hearing Status** — סטטוס דיון: 0=פעיל, 1=מבוטל, 2=הועבר.
- **EffectiveCourtCity** — נתון מדיון לצורך gating בלבד. עמודת שם בית משפט (text_mkxez28d) ממופה מ-UserData "שם בית משפט".
